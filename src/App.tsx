...
export default function App() {
  ...
  const exportRoutineAsJson = (routine: Routine) => {
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(routine, null, 2));
    const downloadAnchorNode = document.createElement('a');
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute("download", `${routine.name.replace(/\s+/g, '_')}.json`);
    document.body.appendChild(downloadAnchorNode);
    downloadAnchorNode.click();
    downloadAnchorNode.remove();
  };

  const exportRoutineAsArduino = (routine: Routine) => {
    let code = `// Arduino Routine: ${routine.name}\n`;
    code += `// Generated by RoboControl Studio\n\n`;
    code += `const int NUM_STEPS = ${routine.steps.length};\n`;
    code += `const int NUM_SERVOS = 6;\n\n`;
    code += `// {s1, s2, s3, s4, s5, s6, duration}\n`;
    code += `const int routine[NUM_STEPS][NUM_SERVOS + 1] PROGMEM = {\n`;
    
    routine.steps.forEach((step, idx) => {
      const angles = step.angles.map(a => Math.round(a)).join(', ');
      code += `  {${angles}, ${step.duration}}${idx < routine.steps.length - 1 ? ',' : ''}\n`;
    });
    ...
    const downloadAnchorNode = document.createElement('a');
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute("download", `${routine.name.replace(/\s+/g, '_')}.ino`);
    document.body.appendChild(downloadAnchorNode);
    downloadAnchorNode.click();
    downloadAnchorNode.remove();
  };
  ...
  const handleImportRoutine = () => {
    ...
    input.onchange = (e: any) => {
      ...
      reader.onload = (event: any) => {
        try {
          const routine = JSON.parse(event.target.result);
          if (routine.steps && Array.isArray(routine.steps)) {
            saveRoutines([...customRoutines, { ...routine, id: Math.random().toString(36).substr(2, 9) }]);
          }
        } catch (err) {
      ...
      <main className="p-6 grid grid-cols-12 gap-6 max-w-[1600px] mx-auto">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-6">
          ...
          <section className="space-y-3">
            ...
            <ArmVisualizer servos={servos} isRecording={isRecording} />
          </section>
            ...
            <section className="space-y-3">
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden h-[400px] flex flex-col">
                <Tabs defaultValue="premade" className="flex-1 flex flex-col">
                  <TabsList className="bg-zinc-950/50 border-b border-zinc-800 rounded-none h-12 px-2 flex justify-between items-center">
                            ...
                            <div key={routine.id} className="group p-3 bg-zinc-950 border border-zinc-800 rounded-lg hover:border-zinc-600 transition-all flex items-center justify-between">
                              ...
                              <div className="flex gap-1">
                                ...
                                <Button 
                                  size="icon" 
                                  variant="ghost" 
                                  className="h-8 w-8 rounded-full hover:bg-zinc-800 text-zinc-400 hover:text-white transition-all"
                                  onClick={() => exportRoutineAsJson(routine)}
                                  title="Export JSON"
                                >
                                ...
                                <Button 
                                  ...
                                  variant="ghost" 
                                  className="h-8 w-8 rounded-full hover:bg-zinc-800 text-zinc-400 hover:text-white transition-all"
                                  onClick={() => exportRoutineAsArduino(routine)}
